...
from src.utils import EarlyStopping, train, train_trans, train_no_early_stopping, train_trans_no_early_stopping
...
def run_task_umap(suite_id: int, task_id: int, result_folder: str, seed: int = 10):
    ...
    X, y, categorical_indicator, attribute_names = load_dataset_offline(suite_id, task_id)
    ...
    def boosted(trial):
        ...
        RMSE_boost=np.sqrt(np.mean((y_val.values-y_val_hat_boost)**2))
    ...
    def rf(trial):
        ...
        RMSE_rf=np.sqrt(np.mean((y_val.values-y_val_hat_rf)**2))
    ...
    RMSE_boosted=np.sqrt(np.mean((y_test.values-y_test_hat_boosted)**2))

    rf_model.fit(X_train, y_train)
    y_test_hat_rf=rf_model.predict(X_test)
    RMSE_rf=np.sqrt(np.mean((y_test.values-y_test_hat_rf)**2))

    lin_reg=LinearRegression()
    lin_reg.fit(X_train, y_train)
    y_test_hat_linreg=lin_reg.predict(X_test)
    RMSE_linreg=np.sqrt(np.mean((y_test.values.flatten()-y_test_hat_linreg)**2))

    constant_prediction = np.array([np.mean(y_train)]*len(y_test))
    RMSE_constant = np.sqrt(np.mean((y_test.values - constant_prediction) ** 2))
    ...
    def gam_model(trial):
        ...
        RMSE_gam = np.sqrt(np.mean((y_val.values - y_val_hat_gam) ** 2))
    ...
    RMSE_results = {
        'constant':           RMSE_constant,            # constant baseline
        'MLP':                RMSE_MLP.item(),         # convert torch.Tensor to float
        'ResNet':             RMSE_ResNet.item(),
        'FTTrans':            RMSE_FTTrans.item(),
        'boosted_trees':      RMSE_boosted,
        'rf':                 RMSE_rf,
        'linear_regression':  RMSE_linreg,
        'engression':         RMSE_engression.item(),
        'GAM':                RMSE_gam
    }

    # ─── (2) Convert to DataFrame ───────────────────────────────────────────────────
    df_all = pd.DataFrame(
        list(RMSE_results.items()),
        columns=['Method', 'RMSE']
    )
    ...
    # ─── (4) Write one CSV with “<suite_id>_<task_id>_spatial_depth_RMSE_results.csv” ─
    output_path = os.path.join(
        result_folder,
        f"{suite_id}_{task_id}_spatial_depth_RMSE_results.csv"
    )
    df_all.to_csv(output_path, index=False)
    print(f"Saved all‐methods RMSEs to {output_path}")
