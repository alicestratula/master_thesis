...
def main():
    ...
    for split_fn in methods:
        ...
        is_extrap = (split_fn is not random_split)
        ...
        def obj_crps_gam_pf(trial):
            if not (is_reg and is_extrap):
                return float('inf')  # skip non-extrap/reg
            ...
            try:
                ...
                pr = gam.predict_parameters(X_val)
                return float(np.mean(crps_gaussian(y_val, pr['loc'], pr['scale'])))
            except Exception:
                return float('inf')

        def obj_rmse_gam_pf(trial):
            if not (is_reg and is_extrap):
                return float('inf')
            n_splines = trial.suggest_int('n_splines', 10, 100)
            spline_order = trial.suggest_int('spline_order', 1, 5)
        ...
        def obj_logloss_gam_pf(trial):
            if not (not is_reg and is_extrap):
                return float('inf')
            n_splines = trial.suggest_int('n_splines', 10, 100)
            spline_order = trial.suggest_int('spline_order', 1, 5)
            lam = trial.suggest_float('lam', 1e-3, 1e3, log = True)
            try:
                ...
                probs = gam.predict_proba(X_val)
                return float(evaluate_log_loss(y_val, probs))
            except Exception:
                return float('inf')

        def obj_accuracy_gam_pf(trial):
            if not (not is_reg and is_extrap):
                return float('-inf')
            n_splines = trial.suggest_int('n_splines', 10, 100)
            spline_order = trial.suggest_int('spline_order', 1, 5)
        ...
        if is_extrap:
            if is_reg:
                tasks.append(("GAM_CRPS", obj_crps_gam_pf, 'minimize', 'CRPS', GAMRegressor))
                tasks.append(("GAM_RMSE", obj_rmse_gam_pf, 'minimize', 'RMSE', GAMRegressor))
            else:
                tasks.append(("GAM_ll", obj_logloss_gam_pf, 'minimize', 'LogLoss', GAMClassifier))
                tasks.append(("GAM_acc", obj_accuracy_gam_pf, 'maximize', 'Accuracy', GAMClassifier))
        ...
        for name, (study, metric, ModelClass) in studies.items():
            ...
            try:
                ...
                print(f"Skipping {name} due to ValueError in best_params")
                continue
            if name.startswith("GAM"):
                n_splines = best["n_splines"]
                spline_order = best["spline_order"]
                lam = best["lam"]
            ...
            out_tr = y_tr_arr if not isinstance(model, DistributionalRandomForestRegressor) else y_tr_arr
            try:
                model.fit(inp_tr, out_tr)
            except np.linalg.LinAlgError as e:
                print(f"Skipping GAM {name_split}/{name} because SVD failed on final fit: {e}")
                continue
            except RuntimeError as e:
                if "out of memory" in str(e).lower():
                    print(f"Skipping GAM {name_split}/{name} due to out of memory error")
                    continue
                else:
                    raise
            ...
            if metric == 'CRPS':
                ...
                preds = model.predict(
                    X_te_p if not isinstance(model, LightGBMLSSRegressor) else X_te_p.to_numpy()
                )
