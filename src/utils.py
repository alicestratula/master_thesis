        ...
        for X, y in train_loader:
            ...
            if  isinstance(criterion, torch.nn.CrossEntropyLoss):
                ...
                if flat_y.max() >= num_classes or flat_y.min() < 0:
                    print(f"Number of classes (from model output): {num_classes}")
                    print(f"Labels tensor (flat_y) shape: {flat_y.shape}")
                    print(f"Labels tensor dtype: {flat_y.dtype}")
                    print(f"Min label value: {flat_y.min().item()}")
                    print(f"Max label value: {flat_y.max().item()}")
                    # Find and print the specific offending labels
                    offending_indices = (flat_y >= num_classes) | (flat_y < 0)
                    print(f"Offending labels: {flat_y[offending_indices]}")
                    # You might want to stop execution here to fix the data
