...
def run_task_umap(suite_id: int, task_id: int, result_folder: str, seed: int = 10):
    ...
    def gam_model(trial):
        ...
        RMSE_gam = np.sqrt(np.mean((y_val.to_numpy() - y_val_hat_gam) ** 2))
    ...
    # Calculate the RMSE
    RMSE_gam = np.sqrt(np.mean((y_test.to_numpy() - y_test_hat_gam) ** 2))
    ...
    df.to_csv(os.path.join(result_folder, f"{suite_id}_{task_id}_spatial_depth.csv"), index=False)
